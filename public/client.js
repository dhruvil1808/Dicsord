...
const sendMessage = () => {
  ...
  if (info.length > 0) {
    ...
    appendMessage(data, "row justify-content-end message_container"); //appending data to message area height-fit-content
    scrolltoBottom();
  }
};
function appendMessage(data, type) {
  ...
  maindiv.innerHTML = `
     <div class="card" id="message"> 
      <h4 style="font-size:20px">${data.username}</h4>
      <p style="font-size:25px;padding:0%">${data.info}</p>
      </div>
